...
pid_t fork() {
	/*TODO:call syscall*/
	int pid_t = syscall(SYS_FORK,0,0,0,0,0);
	//printf("%d\n", pid_t);
	return pid_t;
	//return 0;
}

int exec(uint32_t sec_start, uint32_t sec_num, char* const argv[]){
	/*TODO:call syscall*/
	return syscall(SYS_EXEC,sec_start, sec_num, (uint32_t)argv,0,0 );
	//return 0;
}
