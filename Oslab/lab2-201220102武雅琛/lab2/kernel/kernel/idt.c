...
void initIdt() {
	...
	//setTrap(idt + 0, SEG_KCODE, (uint32_t)irqErrorCode,DPL_KERN);
	setTrap(idt + 8, SEG_KCODE, (uint32_t)irqDoubleFault,DPL_KERN);
	setTrap(idt + 10, SEG_KCODE, (uint32_t)irqInvalidTSS,DPL_KERN);
	setTrap(idt + 11, SEG_KCODE, (uint32_t)irqSegNotPresent,DPL_KERN);
	setTrap(idt + 12, SEG_KCODE, (uint32_t)irqStackSegFault,DPL_KERN);
	setTrap(idt + 13, SEG_KCODE, (uint32_t)irqGProtectFault,DPL_KERN);
	setTrap(idt + 14, SEG_KCODE, (uint32_t)irqPageFault,DPL_KERN);
	setTrap(idt + 17, SEG_KCODE, (uint32_t)irqAlignCheck,DPL_KERN);
	setTrap(idt + 30, SEG_KCODE, (uint32_t)irqSecException,DPL_KERN);
