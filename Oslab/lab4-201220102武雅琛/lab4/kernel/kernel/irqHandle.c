...
void syscallmodifyCount(struct StackFrame *sf);
void syscallReadCount(struct StackFrame *sf);
void syscallCount(struct StackFrame *sf);
...
void timerHandle(struct StackFrame *sf) {
	...
	if (pcb[current].state == STATE_RUNNING &&
		...
		{
			putString("schedule:");putChar(current + '0');putString("to");putChar(i + '0');putChar('\n');
...
void syscallReadStdIn(struct StackFrame *sf) {
	...
	if(dev[STD_IN].value<0){
		...
		//TODO
		dev[STD_IN].value = dev[STD_IN].value - 1;
		...
		//putString("wake!\n");
		//disableInterrupt();//critical section
		int sel = sf->ds;
		char *str = (char *)sf->edx;
		asm volatile("movw %0, %%es"::"m"(sel));
		int i = 0;
		//int len = (bufferTail - bufferHead + MAX_KEYBUFFER_SIZE) % MAX_KEYBUFFER_SIZE;
		while(bufferHead != bufferTail && i < sf->ebx)
		{
			char character = keyBuffer[bufferHead];
			putString("obatin character:");
...
void syscallSemInit(struct StackFrame *sf) {
	...
	if (i < MAX_SEM_NUM)
	{
        ...
        sem[i].value = sf->edx; // >=0: no process blocked; -1: 1 process blocked; -2: 2 process blocked;...
		sf->eax = i;
		return;
	}
	sf->eax = -1;
...
void syscallSemWait(struct StackFrame *sf) {
	...
	putString("P operation:");putInt(current);putString("sem:");putChar(i + '0');putString("value:");putChar(sem[i].value + '0');putChar('\n');
	if (sem[i].value < 0)
	{
		putChar(current + '0');putString(" wait on ");putInt(i);
		pcb[current].blocked.next = sem[i].pcb.next;
    	pcb[current].blocked.prev = &(sem[i].pcb);
    	sem[i].pcb.next = &(pcb[current].blocked);
    	(pcb[current].blocked.next)->prev = &(pcb[current].blocked);
...
void syscallSemPost(struct StackFrame *sf) {
	...
	sem[i].value = sem[i].value + 1;
	putString("V operation:");putInt(current);putString("sem:");putChar(i + '0');putString("value:");putChar(sem[i].value + '0');putChar('\n');
	if (sem[i].value <= 0)
	{
		...
		putChar(pt->pid + '0');putString(" wake on ");putInt(i);
		pt->state=STATE_RUNNABLE;
		asm volatile("int $0x20");
		sf->eax = 0;
	}
...
void syscallSemDestroy(struct StackFrame *sf) {
	...
	pt = (ProcessTable*)((uint32_t)(sem[i].pcb.prev));
    if (pt != (ProcessTable*)&(sem[i].pcb))//exist precess blocked by the samphore
	{
		sf->eax = -1;
		return;
	}
...
void syscallmodifyCount(struct StackFrame *sf)
{
	...
	{
		sf->eax = -1;
		putString("Error: count = ");putInt(count);
	}
	else sf->eax = count;
	return;
}
